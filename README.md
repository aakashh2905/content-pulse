...
User opens ASPI Content Pulse.
User sees the app status: OpenAI ready, Apify ready, storage mode.
User enters the client or brand name.
User enters a keyword.
User optionally adds competitor handles.
User chooses industry, region, goal, and output language.
App recommends Run all skills.
User clicks run.
App shows loading state with current stage.
App displays result cards.
App displays topic recommendation.
App displays script and hooks.
User copies or exports the output for internal review.
Run is saved in history.
Acceptance criteria:
...
User enters a broad or weak keyword.
App normalizes spelling and expands related hashtag seeds.
App tries primary and broadened search strategies.
If no results are found, app shows stronger seed suggestions.
User can click or copy a suggested seed.
User reruns the search.
Acceptance criteria:
...
User enters keyword set.
User selects run schedule.
User saves campaign.
Campaign appears in saved campaigns.
User can run now, pause, activate, or delete.
Acceptance criteria:
...
Manager opens run history.
Manager selects a previous run.
Manager sees summary, recommended topic, source posts, generated script, and hooks.
Manager can approve, reject, or request edits.
Team can export report.
Acceptance criteria:
...
Approval workflow.
These requirements convert ASPI Content Pulse from a technical prototype into a user-ready internal content intelligence product.ASPI Content Pulse Product Requirements V2
Date: 5 May 2026
